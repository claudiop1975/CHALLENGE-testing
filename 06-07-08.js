...
function crearClasePersona() {
  class Persona {
    ...
    addFriend(nombre, edad) {
      ...
      // Tu código aca:
      let amicchi = {nombre, edad};
      this.amigos.push(amicchi);
    ...
    getFriends() {
      ...
      let ammi=[]
        for(let i = 0; i < this.amigos.length; i++) {
          ammi.push(this.amigos[i]['nombre'])
        }
        return ammi;
    ...
    getPromedioEdad() {
      ...
      let fpe= 0;
        for(let i = 0; i < this.amigos.length; i++) {
          pedad +=(this.amigos[i]['edad'])
          fpe = pedad/(this.amigos.length)
        }
        return fpe;
    }
  };
